...
def get_timezone():
    """ Select best timeZone """
    # detect if the incoming request contains timezone argument
    request_timezone = request.args.get('timezone')
    user = g.user

    if request_timezone:
        _timezone = request_timezone
    elif user and user.get("timezone"):
    ...
    return request.accept_languages.best_match(app.config["BABLE_DEFAULT_TIMEZONE"])
